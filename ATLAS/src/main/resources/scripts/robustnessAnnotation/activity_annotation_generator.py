...
def main():
    dirs = ('ScenarioReview2', 'ScenarioReview', 'storyboard1')
    for dir in dirs:
        activities = read_activity_map()
        current_dir = os.path.dirname(os.path.realpath(__file__))
        resources_dir = os.path.dirname(os.path.dirname(current_dir))
        fragments_dir = os.path.join(resources_dir, dir, 'fragments')
        processes_dir = os.path.join(resources_dir, dir, 'processes')
        robustness_dir = os.path.join(resources_dir, dir, 'robustness')
        new_activities = dict()
